class User < ApplicationRecord
  ...
  validates :nick_name, format: { with: /\A[a-zA-Z0-9一-龯ぁ-んァ-ンー－]+\z/, message: "は英語、数字、日本語、カタカナ、ひらがなのみで設定してください" }
  validates :email, presence: true
  validates :encrypted_password, format: { with: /\A(?=.*?[a-zA-Z])(?=.*?[0-9]).{6,}\z/, message: "は英字と数字を含む6文字以上で設定してください" }
